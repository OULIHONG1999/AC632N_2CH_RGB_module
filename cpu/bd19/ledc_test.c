...
static u8 led_rgb_val_buf0[3 * LED_NUM_MAX] __attribute__((aligned(4)));
static u8 led_rgb_val_buf1[3 * LED_NUM_MAX] __attribute__((aligned(4)));
...
#define led_send_rgbbuf(ch, buf, num) ledc_send_rgbbuf_isr(ch, buf, num, 1)
...
u8 led_cnt_to_rgb_val(struct led_rgb_t *led_rgb, int colour, int cnt)
{
    ...
    if ((led_rgb->pu_cnt_start[i] <= cnt) && (cnt < led_rgb->pu_cnt_end[i]))
    { // 递增区间
    ...
    else if ((led_rgb->keep_cnt_start[i] <= cnt) && (cnt < led_rgb->keep_cnt_end[i]))
    { // 高维持区间
        rgb_val = led_rgb->rgb_val_max[i];
    }
    else if ((led_rgb->pd_cnt_start[i] <= cnt) && (cnt < led_rgb->pd_cnt_end[i]))
    { // 递减区间
 ...
 * @param cnt 基础节拍计数值
 */
void led_rgb_upgrade(struct led_rgb_t *led_rgb, int cnt, char ch)
{
    u8 r_val = 0;
    u8 g_val = 0;
    u8 b_val = 0;
    if ((cnt % led_rgb->cnt_freq) == 0)
    {
        ...
        for (int i = 0; i < led_rgb->div_sec_num; i++)
        {
            ...
            for (int j = 0; j < led_rgb->sec_led_num; j++)
            {
                ...
                if (ch == 0)
                    led_rgb_to_buf(r_val, g_val, b_val, led_rgb_val_buf0, i * led_rgb->sec_led_num + j);
                else if (ch == 1)
                    led_rgb_to_buf(r_val, g_val, b_val, led_rgb_val_buf1, i * led_rgb->sec_led_num + j);
            }
        }
        if (ch == 0)
            led_send_rgbbuf(ch, led_rgb_val_buf0, led_rgb->div_sec_num * led_rgb->sec_led_num);
        else if (ch == 1)
            led_send_rgbbuf(ch, led_rgb_val_buf1, led_rgb->div_sec_num * led_rgb->sec_led_num);
    }
}
...
void led_rgb_scan(void *priv)
{
    ...
    led_rgb_upgrade(led_rgb, step_cnt, 0);
    step_cnt++;
}
...
static struct led_rgb_t *led_rgb0 = NULL;
static struct led_rgb_t *led_rgb1 = NULL;
void rgb_ch0_timer()
{
    if (led_rgb0 == NULL)
        return;
    ...
    led_rgb_upgrade(led_rgb0, step_cnt, 0);
    step_cnt++;
}
...
void rgb_ch1_timer()
{
    ...
    led_rgb_upgrade(led_rgb1, step_cnt, 1);
    step_cnt++;
}

static u16 timer0_id = 0;
static u16 timer1_id = 0;
...
void set_led_rgb_table(char ch, char mode_idx)
{
    ...
    // 模式大于0，则启动定时器，并根据模式启动不同的灯光效果
    if (mode_idx > 0)
    {
        if (mode_idx >= (sizeof(led_rgb_table) / 4))
        {
            printf("mode_idx error\n");
            return;
        }
        printf("start led rgb timer %d\n", ch);
        switch (ch)
        {
        case 0:
            led_rgb0 = led_rgb_table[mode_idx - 1];
            if (timer0_id == 0)
            {
                timer0_id = sys_timer_add(NULL, rgb_ch0_timer, 10);
                printf("timer0_id %d\n", timer0_id);
            }

            break;
        case 1:
            led_rgb1 = led_rgb_table[mode_idx - 1];
            if (timer1_id == 0)
            {
                timer1_id = sys_timer_add(NULL, rgb_ch1_timer, 10);
                printf("timer1_id %d\n", timer1_id);
            }
    ...
    {
        printf("stop led rgb timer %d\n", ch);
        switch (ch)
        {
        case 0:
            sys_timer_del(timer0_id);
            timer0_id = 0;
            break;
        case 1:
            sys_timer_del(timer1_id);
            timer1_id = 0;
            break;
        }
    }
